<script lang="ts">
	...
	// Redirect function for 'register' route
	function redirectToRegister() {
		goto('register');
	}

	// Initialize superForm with options
	const { enhance, form, errors, message } = superForm(data.userLoginFormData, {
		...
		onUpdated: () => {
			if (!message) return; // Avoid errors if message is undefined

			const { alertType, alertText } = message;

			// Trigger SweetAlert based on alert type
			if (alertType === 'error') {
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
	...
	// Reactively display SweetAlert2 error if form result fails
